...
class astrolib{
	...
	static getBodyRaDec(jd,bodyNum,lat,lon,precess){
		...
		//TODO: Convert jdTT to TDB

		let t = astrolib.convertJDToJulianMilleniaSinceJ2000(jdTT);
		
		//Get current position of Earth
		const earth = astrolib.getBody(3,jdTT);

		let body=astrolib.getBodyLightTimeAdjusted(jdTT,earth,bodyNum);
	...
	static getBodyLightTimeAdjusted(jd,origin,bodyNum){
		//Get current position of body
		let body = astrolib.getBody(bodyNum,jd);
		...
		for(let i=0;i<2;i++){
			...
			distance*=1000; //Convert from AU to meters
			const lightTime=distance/299792458.0;

			//Convert light time to Days, and subtract it from the original value of t
			newT-=lightTime / 24.0 / 60.0 / 60.0 ;  
	...
	static getBody(bodyNum,jd){
		const de=new DE405();
		switch(bodyNum){
			case 0: 
				return(de.getAllPropertiesForSeries(10,jd));
			case 1:
				return(de.getAllPropertiesForSeries(0,jd));
			case 2:
				return(de.getAllPropertiesForSeries(1,jd));
			case 3:
				const earth1=DE405.getEarthPositionFromEMB(de.getAllPropertiesForSeries(2,jd),de.getAllPropertiesForSeries(9,jd))
				//console.log(earth1)
				return earth1;
			case 4:
				return(de.getAllPropertiesForSeries(3,jd));
			case 5:
				return(de.getAllPropertiesForSeries(4,jd));
			case 6:
				return(de.getAllPropertiesForSeries(5,jd));
			case 7:
				return(de.getAllPropertiesForSeries(6,jd));
			case 8:
				return(de.getAllPropertiesForSeries(7,jd));
			case 9:
				return(de.getAllPropertiesForSeries(8,jd));
			case 10:
				const moon=de.getAllPropertiesForSeries(9,jd);
				const earth=DE405.getEarthPositionFromEMB(de.getAllPropertiesForSeries(2,jd),moon);
				return astrolib.add(moon,earth)
